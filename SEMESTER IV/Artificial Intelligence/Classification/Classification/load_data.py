...
def generate_data():
    TRAIN_SAMPLES = 100
    TEST_SAMPLES = 20
    FEATURE_DIMENSION = 5  # the dimension of the feature vector

    X_train = np.random.rand(TRAIN_SAMPLES, FEATURE_DIMENSION)
    y_train = np.random.binomial(1, 0.5, TRAIN_SAMPLES)
    X_test = np.random.rand(TEST_SAMPLES, FEATURE_DIMENSION)
